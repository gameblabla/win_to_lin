...
#define print_vd_info_tmp(title, fn)                \
  psz_str = fn(&pvd);                           \
  if (psz_str) {                                \
    report(stdout, title ": %s\n", psz_str);    \
  }
  ...
  case CDIO_FS_ISO_UDF:
...

    {
      ...
      if ( iso9660_fs_read_pvd(p_cdio, &pvd) ) {
        ...
        print_vd_info_tmp("Volume     ", iso9660_get_volume_id);
        tmpfp = fopen("/tmp/cdrom_current.txt", "w");
        fprintf(tmpfp, "%s", psz_str);
        fclose(tmpfp);
		free(psz_str);
		psz_str = NULL;
